								...
								<table id="bloodpressure" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th>心率</th>
											<th>结果分析</th>
											<?php if($vo['report_url'] != null): ?><th>报告图片</th><?php endif; ?>
										</tr>
									</thead>
									<tbody>
										...
										<?php if(is_array($reportArray)): $i = 0; $__LIST__ = $reportArray;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="report">
												...
												<td><?php echo ($vo["blood_oxygen"]); ?></td>
												<td><?php echo ($vo["heartbeat"]); ?></td>
												<td><?php echo ($vo["content"]); ?></td>
												<!-- <td><?php echo ($vo["reference_range"]); ?></td> -->
												<?php if($vo['report_url'] != null): ?><td><a href="<?php echo ($vo["report_url"]); ?>">查看详情</a></td><?php endif; ?>

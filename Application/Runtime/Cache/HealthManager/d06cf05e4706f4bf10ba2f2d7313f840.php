	...
	<div class="col-xs-12">
		<div class="box">
			...
			<div class="box-body oneView">
				...
				<div class="col-sm-2" style="float: right; margin-bottom: 20px;">
					<div class="btn-group">
						...
						<ul class="dropdown-menu" role="menu">
							<?php if(is_array($departmentInfo)): $i = 0; $__LIST__ = $departmentInfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
									<a href="javascript:;" did='<?php echo ($vo["department_id"]); ?>' class="did"><?php echo ($vo["department_name"]); ?></a>
								</li><?php endforeach; endif; else: echo "" ;endif; ?>
										...
										success : function($data){
											if($data.status == 0){
												...
												$.each($data, function(i, result) {
													...
													item += '<td class="ins"><input type="radio" name="uid"  class="check"  value="'+result.uid+'"/></td>';
													item += '</tr>';
												});
												
												$('#postAskCount').html(item);
											}
										}
									});
				...
				<form action="<?php echo U('postAskContent');?>" method="post" id="postAskCount" enctype="multipart/form-data">
					<tbody class="content">
						<?php if($doctorListArray != NULL): if(is_array($doctorListArray)): $i = 0; $__LIST__ = $doctorListArray;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="checkMe">
									<!--选框-->
									<td><img src="<?php echo ($vo["avatar"]); ?>" style="width: 80px"></td>
									<td><?php echo ($vo["tinyName"]); ?></td>
									<td><?php echo ($vo["positionName"]); ?></td>
									<td><?php echo ($vo["hospitalName"]); ?></td>
									<td><?php echo ($vo["departmentName"]); ?></td>
									<td><?php echo ($vo["skill"]); ?></td>
									<td class="ins">
										<input type="radio" name="uid"  class="check"  value=<?php echo ($vo["uid"]); ?> />
									</td>
								</tr><?php endforeach; endif; else: echo "" ;endif; ?> 
						<?php else: ?>
							<p style="font-size: 14px; color: #666; text-align: center; height: 200px; line-height: 200px;" class="noShop">对不起，该科室下暂时没有医生！</p><?php endif; ?>
					</tbody>
				</table>
			...
			<div class="box-body twoView">
					...
					<div class="form-group">
						...
						<div class="col-sm-8" style="margin-bottom: 15px;">
							<input type="button" value="确定咨询" class="btn btn-default btn-flat" onclick="savaSon();"/>
						</div>
					</div>
					<button type="button"
						style="float: left; margin-top: 20px; margin-left: 10px;"

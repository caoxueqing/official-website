function setImageSrcByUrl(src,url){

    var imageEletments = document.getElementsByTagName("img");
    
 
    for (i = 0; i < imageEletments.length; i++)
    {
        
        element=imageEletments[i];


          var imageUrl=element.getAttribute("url");
 
 
        
        if(imageUrl == url){
            element.setAttribute("status","downloaded");
...
function downloadAllImages(){
   ...
   var imageEletments = document.getElementsByTagName("img");
    ...
    for (i = 0; i < imageEletments.length; i++)
    {
        ...
        location.href="downimage://" + element.getAttribute("url");
        //element.add
    }
...
function download_image(element){
    ...
    if(element.getAttribute("status")=="loading" || element.getAttribute("status")=="loaded"){
        
    }
    else{
        element.onclick = function() { return false; };
        element.innerHTML="<span> 下载中...</span>";
        element.setAttribute("status","loading");
        location.href="downimage://" + element.getAttribute("url");
    }
...
function onLoad(){
    ...
    for (i = 0; i < imageEletments.length; i++)
    {
        ...
        var realHeight=height*elementWidth/width;

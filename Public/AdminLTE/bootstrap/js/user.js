...
//    咨询列表弹窗 - 根据type显示不同弹窗
function userAsklist(event){
	var bs = $(event.target);
	var type = bs.data('type');
	switch (type) {
		case 1:
			$('#advisoryReply').modal('show');
			break;
		default:
	}
}
...
//    公共部分用户登陆 - 根据type显示不同弹窗
function headerOn(event){
	var bs = $(event.target);
	var type = bs.data('type');
	switch (type) {
		case 1:
...
function userList(event) {
	...
	switch (type) {
	case 1:
		$('#uCreateUserModel').modal('show');
		break;
	case 2:
		$('#iCreateUserModel').modal('show');
		break;
	default:
	}
}	
